...
print(time1)
print(time2)
dem = [time1[i] for i in range(len(time1))]
payme = [time2[i] for i in range(len(time2))]
br1 = np.arange(len(dem))
br2 = [i + barwidth for i in br1]

plt.bar(br1,dem,color='r',width=barwidth,edgecolor='grey',label='Proposed auction')
plt.bar(br2,payme,color='g',width=barwidth,edgecolor='grey',label='Dummy auction')

plt.xlabel('No of model owners (M)',fontsize=30)
